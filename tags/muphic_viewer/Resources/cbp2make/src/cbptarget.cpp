...
{
 return AutoFileExtension(CPlatform::OS(Platform));
}
 ...
 const CString& ProjectOptions, const CString& TargetOptions)
{
 switch (Relation)
 {
  ...
  case CBuildTarget::orTargetProject: return TargetOptions+ProjectOptions;
  default:
  case CBuildTarget::orProjectTarget: return ProjectOptions+TargetOptions;
 }
}
...
CString CBuildTarget::IncDirs(void)
{
 CString result;
 for (int i = 0; i < m_CompilerDirectories.GetCount(); i++)
 {
  result += " -I"+m_CompilerDirectories[i];
 }
 return result;
}
...
{
 return MakeOptions(m_IncludeDirectoriesRelation,ProjectIncDirs,IncDirs());
}

CString CBuildTarget::ResDirs(void)
{
 CString result;
 for (int i = 0; i < m_ResourceCompilerDirectories.GetCount(); i++)
 {
  result += " -I"+m_ResourceCompilerDirectories[i];
 }
 return result;
}
...
{
 return MakeOptions(m_ResourceIncludeDirectoriesRelation,ProjectResDirs,ResDirs());
}

CString CBuildTarget::LibDirs(void)
{
 CString result;
 for (int i = 0; i < m_LinkerDirectories.GetCount(); i++)
 {
  result += " -L"+m_LinkerDirectories[i];
 }
 return result;
}
...
{
 return MakeOptions(m_LibraryDirectoriesRelation,ProjectLibDirs,LibDirs());
}

CString CBuildTarget::Libs(const CPlatform::OS_Type OS)
{
 CString result;
 for (int i = 0; i < m_LinkerLibraries.GetCount(); i++)
 {
  ...
  if (lib_ext==AutoFileExtension(OS))
  {
   result += " -l"+lib_name;
  }
  else
