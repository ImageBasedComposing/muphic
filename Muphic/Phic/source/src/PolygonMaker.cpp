...
std::pair<int,int> PolygonMaker::checkDirection(int i, int j, Mask* m, std::pair<int,int> ip, int* dir, bool* isVertex)
{
	...
	*isVertex = false;
	...
	if(*dir != -361)
	{
		...
		{
			for(int k = i; k >= 0 && k < i - VAR && k != ip.first; k--)
				for(int t = j; t < m->at(i)->size() && t < j + VAR && t != ip.second; t++)
		...
		{
			for(int k = i; k >= 0 && k < i - VAR && k != ip.first; k--)
				for(int t = j; t >= 0 && t < j - VAR && t != ip.second; t--)
					if(m->at(i)->at(j) == 1 && isBorder(i,j,m))
						newVertex == std::make_pair(k,t);
			*newDir = vectorAngle(i,j,newVertex.first,newVertex.second);
		}
		...
		{
			for(int k = i; k < m->size() && k < i + VAR && k != ip.first; k++)
				for(int t = j; t >= 0 && t < j - VAR && t != ip.second; t--)
					if(m->at(i)->at(j) == 1 && isBorder(i,j,m))
						newVertex == std::make_pair(k,t);
			*newDir = vectorAngle(i,j,newVertex.first,newVertex.second);
		...
		{
			for(int k = i; k < m->size() && k < i + VAR && k != ip.first; k++)
				for(int t = j; t < m->at(i)->size() && t < j + VAR && t != ip.second; j++)
					if(m->at(i)->at(j) == 1 && isBorder(i,j,m))
						newVertex == std::make_pair(k,t);
			*newDir = vectorAngle(i,j,newVertex.first,newVertex.second);
		}
	...
	else
	{
		...
		for(int k = i; k < m->size() && k < i + VAR; k++)
			for(int t = j; j < m->at(i)->size() && t < j + VAR; j++)
				if(m->at(i)->at(j) == 1 && isBorder(i,j,m))
					newVertex == std::make_pair(k,t);
